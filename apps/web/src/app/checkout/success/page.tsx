...
export default async function SuccessPage({ searchParams }: { searchParams?: Promise<{ session_id?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl p-8 text-center">
      ...
      {summary ? (
        <p className="mb-2 text-muted-foreground">{summary.email ? `Email: ${summary.email} • ` : ''}Status: {summary.status || '—'}</p>
      ) : (
        <p className="mb-2 text-muted-foreground">We could not resolve additional session details.</p>
      )}
      {summary?.amount_total != null && (
        <p className="mb-6 text-muted-foreground">Total: ${(summary.amount_total / 100).toFixed(2)}</p>
      )}
